...
export class ContextResolver<Shop extends ShopInterface = ShopInterface> {
	...
	public async fromAPI<Payload = unknown>(
		...
		const signature = req.headers.get("onlishop-shop-signature");

		if (signature === null) {
			throw new Error("Missing onlishop-shop-signature header");
		}
	 ...
	 * This is usually a module request from the onlishop admin
	 */
