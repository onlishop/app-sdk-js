...
interface MiddlewareConfigBase {
	...
	/**
	 * Enable the app iframe integration. This will automatically set a cookie to identifiy the onlishop shop and validate the request from a client side application. See appIframeRedirects
	 */
	appIframeEnable?: boolean;

	/**
	 * The relative url of the app iframe scope. All requests matching this will require that the request has an cookie set with the onlishop shop. This cookie will be automatically set by
	 *
	 * @default "/client-api/*"
	 */
...
interface ParametersConfig {
	...
	/**
	 * The secret of the app. When the app is published in the Onlishop Store, the Onlishop Store provides this value.
	 */
	...
	/**
	 * The token cache to use for the HttpClient. This is used to cache the access token for the onlishop shop. If you don't provide a token cache, the HttpClient will use an in-memory cache.
	 */
...
export function configureAppServer(hono: Hono, cfg: MiddlewareConfig) {
	...
	hono.use(cfg.appPath, async (ctx, next) => {
		...
		ctx.header(
			...
			cloned.headers.get("onlishop-app-signature") as string,
		);
	});
