export class WebCryptoHmacSigner {
	...
	async verifyGetRequest(request: Request, secret: string): Promise<void> {
		...
		const signature = url.searchParams.get("onlishop-shop-signature");

		if (signature === null) {
			throw new Error("Missing onlishop-shop-signature query parameter");
		}

		url.searchParams.delete("onlishop-shop-signature");
